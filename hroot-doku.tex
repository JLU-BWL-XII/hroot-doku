...
\begin{itemize}
    \item Herr Weller (HRZ - hat server im Vault): 0170 / 9107300
    \item Herr Mark Lang (Backup): 0641 / 99-13145
    \item Frau Tanja Zopf (HRZ Netzgruppe Hotline): 0641 99 13100
\end{itemize}
...
\section{Monitoring}
\subsection{Webserver monitoring}
Web server is externally monitored via a private vServer by means a simple ping script.
...
Herr Lang creates a daily backup.
...
\begin{lstlisting}                                                
41 5 * * * /usr/local/sbin/hroot-db-backup.sh 
\end{lstlisting}
...
/usr/bin/find /home/tux/backup -name "*.sql" -type f -mtime +30 
echo "exit staus: $?" 
